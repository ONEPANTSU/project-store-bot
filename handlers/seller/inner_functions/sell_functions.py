        ...
        if message.chat.id in projects_in_moderation:
            ...
            if get_need_payment() == 1:
                await message.answer(
                    text=MESSAGES["put_up_for_sale"].format(
                        regular_price=str(int(int(get_regular_sell_price()) / 100))
                        + "₽",
                        vip_price=str(
                            int(
                                int(get_regular_sell_price() + get_vip_sell_price())
                                / 100
                            )
                        )
                        + "₽",
                ...
                await message.answer(
                    text=MESSAGES["put_up_for_sale"].format(
                        regular_price=MESSAGES["free_payment"],
